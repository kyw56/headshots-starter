// /app/interview/common/methodology/page.tsx
"use client";
...
const METHODOLOGY_QUESTIONS = commonQuestions.filter(q => 
  q.category && ["비전", "시사인식", "학업계획", "관심분야"].includes(q.category)
).map(q => q.content);

const METHODOLOGY_THEME = {
...
export default function CommonMethodologyPage() {
  ...
  useEffect(() => {
    if (METHODOLOGY_QUESTIONS.length > 0 && messages.length === 0) {
      setMessages([
        {
          role: 'assistant',
          content: '안녕하세요! 연구방법론 면접을 시작하겠습니다.'
        },
        {
          role: 'assistant',
          content: METHODOLOGY_QUESTIONS[0]
        }
      ]);
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/evaluate-answer', {
        ...
        body: JSON.stringify({
          question: METHODOLOGY_QUESTIONS[currentQuestion],
          answer: userInput.trim(),
          type: 'common-methodology',
        }),
      });
      ...
      setTimeout(() => {
        setFeedback(null);
        if (currentQuestion < METHODOLOGY_QUESTIONS.length - 1) {
          const nextQuestion: Message = {
            role: 'assistant',
            content: METHODOLOGY_QUESTIONS[currentQuestion + 1]
          };
  ...
  return (
    <div className={`min-h-screen ${METHODOLOGY_THEME.light}`}>
      <div className={`bg-gradient-to-r ${METHODOLOGY_THEME.gradient} text-white py-4 px-6 fixed top-0 w-full z-10 shadow-md`}>
        <h1 className="text-xl font-bold">연구방법론 면접</h1>
      </div>
        ...
        <div className="bg-white rounded-lg shadow-lg p-4 min-h-[70vh] flex flex-col">
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              className={`${METHODOLOGY_THEME.primary} text-white ${METHODOLOGY_THEME.hover} disabled:opacity-50 transition-all shadow-sm px-4`}
